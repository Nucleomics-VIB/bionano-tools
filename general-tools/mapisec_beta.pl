...
# create hashes to store molecule IDs
our %bnxhash = ();
our %in1hash = ();
our %in2hash = ();
our %noalign = ();
...
###########################################
# load full BNX data into array and ID-hash
my $countmol = 0;
my $first = 1;
my $cntln = 0;
my $cntali = 0;
...
print STDERR "# loading BNX data into memory\n";
...
while ( my $line = <$BNX> ) {
	...
	# split row to get molecule ID
	@fields = split( /\t/, $line);
	$molid=$fields[1];
	$bnxhash{$molid}++;
	print BNXLST $molid."\n";
	  
    # read three more lines and store in array
    for ( my $d = 1; $d < 4; $d++ ) {
...
# close BNX stream
print STDERR "## finished loading BNX data for ".$countmol." molecules\n";
undef $BNX;
...
# split BNX data based on hashes
print STDERR "# parsing BNX data and saving to files\n";

foreach my $line (@bnxdata) {
	...
	if ( $line =~ /^0\b/ ) {
		...
		# test if molecule is present in BNX
		( exists($bnxhash{$molid}) ) || die "## error, $molid is absent from BNX file!\n";
		
		# add to @data
		push @data, $line;
		# read three more lines
    	for ( my $d = 1; $d < 4; $d++ ) {
        	$line = shift @bnxdata || die "premature end of file";
        	push @data, $line;
   		}
					...
					if ( exists($in1hash{$molid}) ) {
						...
						$cnt1++;
							...
							$cnt2++;
								...
								die "## could not figure out what to do with molecule $molid\n";
								}
		}
	} else {
...
print STDERR "## finished saving to BNX files\n\n";
