...
The perl script **[fastaPadHeadTrailGap.pl](general-tools/fastaPadHeadTrailGap.pl)** adds N-regions's at front and back of **NGScontigs_HYBRID_SCAFFOLD.fasta** sequences based on the **NGScontigs_HYBRID_SCAFFOLD_trimHeadTailGap.coord** file to account for the head and tail trimming done by the pipeline. The resulting sequences have a length identical to the hybrid **Super-Scaffold** shown in Access (or IrysView).
