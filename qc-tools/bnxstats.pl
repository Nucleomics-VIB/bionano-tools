...
my @filtered = grep {$_->[0] >= $minlen*1000 && $_->[2] >= $minsnr} @BigArray;
...
my @bin150200k = grep {$_->[0]>=150000 && $_->[0]<199999 &&
	$_->[2] >= $minsnr} @BigArray;
...
push (@result, "# filtered-molecules: ".$filttot." (".$filteredpc."%)");
push (@result, "# filtered size (Gb): ".(sprintf("%.3f", $filtnucl/1_000_000_000)) );

my @distheader=("molecules", "# of Molecules", "Quantity (Gb)",
	"Bin Mass Fraction(%)", "Labels(/100kb)" );

my $bintitle = "\n# Merged Filtered Size Distribution (Size >= ".
	$minlen."kb, Label SNR >= ".$minsnr.")";
...
my @distheader2 = ("Length Bin", "# of Molecules", "Quantity (Gb)",
	"Bin Mass Fraction(%)", "Labels(/100kb)" );
...
push (@result, "\n# N50 value for filtered molecules (".$filttot.")");
push (@result, "molecules-length-N50 (kb): ".(sprintf("%.1f", $n50_length_filt/1000)) );
...
# print results to STDOUT and to filebordercolor=0 .5 .5
print STDOUT join("\n", @result)."\n";
print OUT join("\n", @result)."\n";
close OUT;
