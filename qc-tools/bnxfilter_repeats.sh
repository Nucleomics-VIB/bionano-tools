...
while getopts "i:t:m:c:h" opt; do
  case $opt in
    i) input=${OPTARG} ;;
    t) tolerance=${OPTARG} ;;
    m) minele=${OPTARG} ;;
...
case ${act} in
	...
	[2]) tag="repeats"; ;;
	[3]) tag="repeatmask"; ;;
	*) echo "not a valid action (1,2,3)"; exit 1 ;;
esac;
...
#   	and repeatStats.txt file summarizing repeat statistics.
...
if [ $? -eq 0 ]; then
	...
	echo "# additional info was written to two files:"
	echo -e "# \'${foldername}/${name}.rmap\' and \'${foldername}/${name}_repeatStats.txt\'"
	echo
	if [ -f "${foldername}/${name}_repeatStats.txt" ]; then
		echo
		echo "# summary"
		cat ${foldername}/${name}_repeatStats.txt
	fi
else
