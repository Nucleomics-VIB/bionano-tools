...
(echo -n "${infasta}"$'\t'; 
	perl $BNG_HSSCRIPTS/calc_cmap_stats.pl ${refcmap} | \
	tr "=" "\t" | transpose -t | head -1 | \
	sed -e 's/ \+/_/g' | sed -e 's/_\b//g' | column -t) > ${results}
(echo -n "${refcmap}"$'\t'; 
	perl $BNG_HSSCRIPTS/calc_cmap_stats.pl ${refcmap} | \
	tr "=" "\t" | transpose -t | tail -1 | column -t) >>  ${results}
(echo -n "${incmap}"$'\t'; 
	perl $BNG_HSSCRIPTS/calc_cmap_stats.pl ${infolder}/output/fa2cmap/${incmap} | \
	tr "=" "\t" | transpose -t | tail -1 | column -t) >> ${results}
(echo -n "filtered NGS"$'\t'; 
	perl $BNG_HSSCRIPTS/calc_cmap_stats.pl ${infolder}/output/align_final/filtered_NGS.cmap | \
	tr "=" "\t" | transpose -t | tail -1 | column -t) >> ${results}
(echo -n "filtered-not-used NGS"$'\t'; 
	perl $BNG_HSSCRIPTS/calc_cmap_stats.pl ${infolder}/output/align_final/filtered_not_used_NGS.cmap | \
	tr "=" "\t" | transpose -t | tail -1 | column -t) >> ${results}
(echo -n "filtered BNG"$'\t'; 
	perl $BNG_HSSCRIPTS/calc_cmap_stats.pl ${infolder}/output/align_final/filtered_BNG.cmap | \
	tr "=" "\t" | transpose -t | tail -1 | column -t) >> ${results}
(echo -n "filtered-not-used BNG"$'\t'; 
	perl $BNG_HSSCRIPTS/calc_cmap_stats.pl ${infolder}/output/align_final/filtered_not_used_BNG.cmap | \
	tr "=" "\t" | transpose -t | tail -1 | column -t) >> ${results}
